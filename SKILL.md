...
- `GET /v1/mesh/az-generator` — cite-only AZ-GENERATOR-1.0 stamps (Cap-7 mesh DNS factory; deep-node → FRONT Node Gate; not callable; `.az` → `.aziel` → pivot; four real hub duplications and three false sites; AZNet+AZBrowser; ≥49 vault; not typed on ICANN DNS). `POST` refuses `AZG-NOT-CALLABLE`.
