...
- `GET /llms.txt` `/ai.txt` `/cite.json` `/bridge.json` `/v1/bridge` `/design-packs/azcorpus.json` `/design-packs/azlibrary.json` — SEMANTIC-BRIDGE-1.0 Growth-ON maps. `public_icann:false`. Named mesh sites azcorpus (download open, upload none) and azlibrary (download open, Plane-A token upload) are designs inside azielcorpuslibrary.net, not a fifth product. Mesh names do not resolve to hubs (`canonical_hub` / `design_of` is design provenance; `resolves_to_hub:false`). Empty Cap-7 claims stay SLOT.
