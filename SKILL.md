...
- `GET /llms.txt` `/ai.txt` `/cite.json` `/bridge.json` `/v1/bridge` `/design-packs/azcorpus.json` `/design-packs/azlibrary.json` — SEMANTIC-BRIDGE-1.0 Growth-ON maps. `public_icann:false`. Mesh name ≠ new product: labels of the four online hubs. Each `/bridge.json` entry has `canonical_hub`, `tip`, `public_icann:false`, `name_may_change:true`. Named mesh sites azcorpus (download open, upload none) and azlibrary (download open, Plane-A token upload) are designs inside azielcorpuslibrary.net. Mesh names do not resolve to hubs (`canonical_hub` / `design_of` is design provenance; `resolves_to_hub:false`). Empty Cap-7 claims stay SLOT.
