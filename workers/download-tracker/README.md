...
GET `/bridge.json` and `/v1/bridge` honest Cap-7 JSON map. Empty Cap-7 → SLOT empty list. `design_of` is hub design provenance only; `resolves_to_hub` is false. Mesh names do not CNAME/redirect to azieleliab.com / corpus / godlock / HDJ.
