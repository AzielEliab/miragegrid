...
GET `/bridge.json` and `/v1/bridge` honest Cap-7 JSON map. Mesh name ≠ new product: relocatable labels of the four online hubs. Each entry has `canonical_hub`, `tip`, `public_icann:false`, `name_may_change:true`. Named mesh sites azcorpus + azlibrary listed as designs (hash-absolute `GET /design-packs/{label}.json`). Empty Cap-7 claims → SLOT. `canonical_hub` / `design_of` is hub design provenance only; `resolves_to_hub` is false. Mesh names do not CNAME/redirect to azieleliab.com / corpus / godlock / HDJ.
