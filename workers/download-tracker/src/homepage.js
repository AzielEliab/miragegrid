...
<body>
  ...
  <script>
    (function () {
      ...
      $("op-mesh").addEventListener("click", function () { api("GET", "/v1/topology"); });
      ...
      (function () {
        ...
        function paintMesh(raw) {
          ...
          $("meshProducts").textContent = "Catalog MCP mesh_* · FragGate slug=mesh · /v1/mesh/* PROXY · QNS-CD-1.0 cross-map · SPLIT THE WIRES · COLD-COPY SURVIVAL · REHEAL · AZ-GENERATOR-1.0 · MIRAGE-GRID-SHIFT-1.0 · AIRGAP-1.0 · PAPER-VAULT-ON-NODE · not AnonBroadcast · not AZMail ring · not a Node Gate · no public qnsd" + extra;
        }
        ...
        async function refreshMesh() {
          try {
            ...
            paintMesh(merged);
            var nodeId = sessionStorage.getItem("miragegrid_mesh_node");
            if (on && nodeId) {
        ...
        $("meshDisable").onclick = async function () {
          sessionStorage.removeItem("miragegrid_mesh_node");
          paintMesh(await meshPost("/v1/mesh/disable", {}));
          refreshMesh();
        };
        $("meshJoin").onclick = async function () {
          var j = await meshPost("/v1/mesh/join", { product: "miragegrid", label: "MirageGrid Worker" });
          var inner = unwrapMesh(j);
          var id = inner.node_id || inner.id || (inner.session && inner.session.node_id);
          if (id) sessionStorage.setItem("miragegrid_mesh_node", String(id));
          paintMesh(j);
          refreshMesh();
        };
        $("meshLeave").onclick = async function () {
          var id = sessionStorage.getItem("miragegrid_mesh_node");
